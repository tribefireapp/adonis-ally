...
class Discord extends OAuth2Scheme {
  ...
  constructor (Config) {
    ...
    utils.validateDriverConfig('discord', config, ['clientId', 'clientSecret', 'redirectUri'])
  ...
  _getInitialScopes (scopes) {
    return _.size(scopes) ? scopes : ['email', 'identify', 'guilds']
  }
  ...
  async _getUserProfile (accessToken, fields) {
    const response = await got(`${this.apiUrl}/users/@me`, {
      headers: {
